...
class Parser:
    ...
    def _parseLabels(self):
        for (line, n) in self._ilines:
            self._parseLabel(line, n)
            if not self._flag:
                return False
        return True

    def _parseLable(self, line, n):
        l = line.split("//")
        l = l[0]
        if len(l) == 0:
            return ""
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            if len(l) == 3:
                if not l[2].isdigit() or l[2][0] == "-":
                    self._flag = False
                    Parser._error("Parser", n, "Invalid push parameter.")
                    return ""
            ...
            if len(l) == 3:
                if not l[2].isdigit() or l[2][0] == "-":
                    self._flag = False
                    Parser._error("Parser", n, "Invalid pop parameter.")
                    return ""
                return "//" + " ".join(l) + "\n" + self._pop(l[1], l[2], n)
            else:
                self._flag = False
