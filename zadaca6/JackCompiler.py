...
class Compiler:
    ...
    def compileClass(self):
        ...
        while True:
            if self._tokenizer.tokenType() != TK_KEYWORD:
                break
            if self._tokenizer.keyword() not in (KW_CONSTRUCTOR, KW_FUNCTION,
                                                 KW_METHOD):
                break
    ...
    def _compileTerm(self):
        ...
        ) == TK_KEYWORD and self._tokenizer.keyword() in (KW_FALSE, KW_TRUE,
                                                          KW_THIS, KW_NULL):
            self._writeXML('keyword', self._tokenizer.keywordStr())
            self._nextToken()
    ...
    def _error(self, error):
        message = '%s line %d:\n  %s\n  %s' % (self._fname,
                                               self._tokenizer.lineNum(),
                                               self._tokenizer.line(), error)
    ...
    def _expectIdentifier(self):
        if not self._tokenizer.tokenType() == TK_IDENTIFIER:
            self._error('Expected <identifier>, got ' +
                        self._tokenizer.tokenTypeStr())
